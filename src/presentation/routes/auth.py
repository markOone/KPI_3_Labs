...
async def register(data: UserRegisterSchema, user_repo: UserRepositoryImpl = Depends(get_user_repository)):
    use_case = RegisterUserUseCase(user_repo)
    try:
        await use_case.execute(username=data.username, email=data.email, password_plain=data.password)
