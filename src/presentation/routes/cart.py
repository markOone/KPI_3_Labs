...
from src.application.use_cases.cart_use_cases import GetCartUseCase, AddToCartUseCase, ClearCartUseCase
...
async def get_cart(cart_repo: CartRepositoryImpl = Depends(get_cart_repository), user: User = Depends(get_current_user)):
...
async def add_item_to_cart(
    ...
    if stock_qty < item_in.quantity:
        raise HTTPException(status_code=400, detail=f"Not enough stock. Available: {int(stock_qty)}")

    cart = await cart_repo.get_by_user_id(user.id)
    if not cart:
        await cart_repo.create(DomainCart(id=0, user_id=user.id, items=[]))
    ...
    return CartItemResponse(id=0, product_id=item_in.product_id, quantity=item_in.quantity)

@router.delete("/clear", status_code=status.HTTP_200_OK)
async def clear_cart(cart_repo: CartRepositoryImpl = Depends(get_cart_repository), user: User = Depends(get_current_user)):
